...
|-------------|------------|
| 🌙 `.afk` | Автоответы, пока тебя нет на месте, с задержкой повтора на чат |
| 🗒 `.save` / `.note` | Сохранённые текстовые заметки, доступные по имени |
| ⏰ `.remind` | Напоминания самому себе через заданное время (`10m`, `2h`, `1d2h30m`, ...) |
| 🛡 Модерация чатов | Встроенные `.ban` / `.mute` / `.kick` / `.promote` / `.restrict` и другие — см. ниже |
| 🖥 `.neofetch` | Карточка в стиле neofetch о машине, на которой крутится Nimbus, с живыми шкалами ЦП/ОЗУ/диска |
| 📈 `.chatstat` | Статистика чата: топ активных с диаграммой, разбивка по медиа, спарклайн активности по часам |
| 👀 `.whois` | Подробная карточка любого юзера, чата или канала — ID, бейджи, счётчики, био |
...
### 🌙 `.afk` / 🗒 заметки / ⏰ напоминания

```text
.afk отошёл, вернусь через час
.unafk

.save привет Привет! Извини за поздний ответ.
.note привет
.notes
.delnote привет

.remind 2h Позвонить в банк
.reminders
.delremind 3
```

`.afk` отвечает по одному разу на чат (задержка через `.config AFK cooldown`, по
умолчанию 10 минут) на личные сообщения и упоминания, пока включён режим АФК, и
выключается сам, как только ты напишешь следующее сообщение. Заметки — это простые
сохранённые фрагменты текста. Напоминания переживают перезапуск и приходят
сообщением в тот чат, где были поставлены.

### 🛡 Модерация чатов

```text
.ban -u @username -t 1d -r спам
.unban -u @username
.kick -u @username
.mute -u @username -t 2h
.unmute -u @username
.promote -u @username -r "Модератор"
...
.dnd
.invite -u @username
.create -g "Новая группа"
.leave
```

Полный набор команд управления группой/каналом, идёт как встроенный модуль (не нужно
ставить через `.dlmod`). Команды, меняющие права в чате, требуют, чтобы у тебя самого
уже было соответствующее право администратора в этом чате — бот не выдаёт себе того,
чего у него нет.
...
```text
.neofetch          # или .sys
.chatstat          # или .cstat — читает .config Stats default_limit сообщений
.chatstat 5000     # прочитать заданное количество
.whois             # или .wi — текущий чат
.whois @username   # юзер, чат или канал по @юзернейму либо id
```

`.neofetch` рисует карточку машины: система, ядро, версии Python и Nimbus, аптайм,
количество ядер, RSS процесса и живые шкалы ЦП/ОЗУ/диска. Полностью перерисовывается
через `.config SysInfo custom_template`. `.chatstat` пробегает по текущему чату и
показывает топ активных со шкалами и процентами, разбивку по типам медиа и спарклайн
активности по часам за сутки. `.whois` работает с аргументом, с ответом на сообщение
или, если не дать ни того ни другого, с текущим чатом.

Модуль каталога [`textfx`](modules/textfx.py) (`.dlmod textfx`) добавляет `.type` —
анимацию печатной машинки — а также `.mock`, `.flip`, `.wide`, `.bubble` и `.spaced`.

### 💻 Выполнение кода

```text
.e 2 + 2                     # Python, внутри процесса
.ec / .ecpp / .ers / .eg     # C, C++, Rust, Go
.enode / .ets                # Node.js, TypeScript (deno)
.ephp / .eruby / .elua       # PHP, Ruby, Lua
.eperl / .ejava / .ehs       # Perl, Java, Haskell
.esh                         # Bash
.ebf ++++++++[>++++...       # Brainf*ck, встроенный
.esql select 1 + 1           # in-memory SQLite, встроенный
.elangs                      # какие тулчейны реально есть на этой машине
```

Любая команда берёт код из аргумента или из сообщения, на которое ты ответил.
Компилируемым языкам нужен установленный тулчейн — `.elangs` покажет, какие есть.
`.ebf`, `.esql` и `.e` не требуют вообще ничего.
...
- `.dlmod` / `.dlm <название>` — установить модуль по имени из каталога
  [`modules/`](modules/) в этом репозитории, либо по прямой ссылке / GitHub blob-ссылке
- [`DEVELOPING.md`](DEVELOPING.md) — как написать свой модуль и добавить его в каталог
  (на английском)
