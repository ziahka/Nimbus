...
class CustomTelegramClient(TelegramClient):
    def __init__(self, *args, **kwargs):
        ...
        self._nimbus_entity_cache: dict[
            str | int,
            CacheRecordEntity,
        ] = {}

        self._nimbus_perms_cache: dict[
            str | int,
            CacheRecordPerms,
        ] = {}

        self._nimbus_fullchannel_cache: dict[
            str | int,
            CacheRecordFullChannel,
        ] = {}

        self._nimbus_fulluser_cache: dict[
            str | int,
            CacheRecordFullUser,
        ] = {}
        ...
        self.nimbus_me: "User"
        self.hikka_me: "User"
        self.nimbus_db: "Database"
        self.loader: "Modules"
        self.nimbus_inline: "InlineManager"
    ...
    @property
    def nimbus_entity_cache(self) -> dict[int, CacheRecordEntity]:
        return self._nimbus_entity_cache

    @property
    def nimbus_perms_cache(self) -> dict[int, CacheRecordPerms]:
        return self._nimbus_perms_cache

    @property
    def nimbus_fullchannel_cache(self) -> dict[int, CacheRecordFullChannel]:
        return self._nimbus_fullchannel_cache

    @property
    def nimbus_fulluser_cache(self) -> dict[int, CacheRecordFullUser]:
        return self._nimbus_fulluser_cache
    ...
    async def get_entity(
        ...
        _nimbus_client_id_logging_tag = copy.copy(self.tg_id)  # noqa: F841
        ...
        if (
            not force
            and hashable_entity
            and hashable_entity in self._nimbus_entity_cache
            and (
                not exp
                or self._nimbus_entity_cache[hashable_entity].ts + exp > time.time()
            )
        ):
            logger.debug(
                "Using cached entity %s (%s)",
                entity,
                type(self._nimbus_entity_cache[hashable_entity].entity).__name__,
            )
            return copy.deepcopy(self._nimbus_entity_cache[hashable_entity].entity)

        resolved_entity = await super().get_entity(entity)

        if resolved_entity:
            cache_record = CacheRecordEntity(hashable_entity, resolved_entity, exp)
            self._nimbus_entity_cache[hashable_entity] = cache_record
            ...
            if getattr(resolved_entity, "id", None):
                ...
                self._nimbus_entity_cache[resolved_entity.id] = cache_record
            ...
            if getattr(resolved_entity, "username", None):
                ...
                self._nimbus_entity_cache[f"@{resolved_entity.username}"] = cache_record
                self._nimbus_entity_cache[resolved_entity.username] = cache_record
    ...
    async def get_perms_cached(
        ...
        if (
            ...
            and hashable_entity
            and hashable_user
            and hashable_user in self._nimbus_perms_cache.get(hashable_entity, {})
            and (
                not exp
                or self._nimbus_perms_cache[hashable_entity][hashable_user].ts + exp
                > time.time()
            )
        ):
            logger.debug("Using cached perms %s (%s)", hashable_entity, hashable_user)
            return copy.deepcopy(
                self._nimbus_perms_cache[hashable_entity][hashable_user].perms
            )
        ...
        if resolved_perms:
            ...
            self._nimbus_perms_cache.setdefault(hashable_entity, {})[
                hashable_user
            ] = cache_record
            ...
            def save_user(key: str | int):
                ...
                if getattr(user, "id", None):
                    self._nimbus_perms_cache.setdefault(key, {})[user.id] = cache_record

                if getattr(user, "username", None):
                    self._nimbus_perms_cache.setdefault(key, {})[
                        f"@{user.username}"
                    ] = cache_record
                    self._nimbus_perms_cache.setdefault(key, {})[
                        user.username
    ...
    async def get_fullchannel(
        ...
        if (
            not force
            and self._nimbus_fullchannel_cache.get(hashable_entity)
            and not self._nimbus_fullchannel_cache[hashable_entity].expired
            and self._nimbus_fullchannel_cache[hashable_entity].ts + exp > time.time()
        ):
            return self._nimbus_fullchannel_cache[hashable_entity].full_channel

        result = await self(GetFullChannelRequest(channel=entity))
        self._nimbus_fullchannel_cache[hashable_entity] = CacheRecordFullChannel(
            hashable_entity,
            result,
            exp,
        )
    ...
    async def get_fulluser(
        ...
        if (
            not force
            and self._nimbus_fulluser_cache.get(hashable_entity)
            and not self._nimbus_fulluser_cache[hashable_entity].expired
            and self._nimbus_fulluser_cache[hashable_entity].ts + exp > time.time()
        ):
            return self._nimbus_fulluser_cache[hashable_entity].full_user

        result = await self(GetFullUserRequest(entity))
        self._nimbus_fulluser_cache[hashable_entity] = CacheRecordFullUser(
            hashable_entity,
            result,
            exp,
        )
    ...
    async def _call(
        ...
        # list and you will be banned from Nimbus federation.
