...
class InlineManager(
    ...
    :type client: nimbus.tl_cache.CustomTelegramClient
    :type db: nimbus.database.Database
    :type allmodules: nimbus.loader.Modules
    """
    ...
    def __init__(
        ...
        self._token = db.get("nimbus.inline", "bot_token", False)
    ...
    async def register_manager(
        ...
        self._name = get_display_name(self._client.nimbus_me)
        ...
        self._bot_client = TelegramClient(
            SQLiteSession(
                os.path.join(main.SESSIONS_DIR, f"nimbus-{self._me}-bot-{bot_uid}")
            ),
            self._client.api_id,
        ...
        try:
            ...
            self._bot_client.nimbus_me = bot_me
        ...
        for folder in _folders.filters:
            if getattr(folder, "title", None) == "Nimbus":
                if any(
                    [
    ...
    async def _ping_bot(
        ...
        try:
            m = await self._client.send_message(self.bot_username, "/start nimbus init")
            ...
            try:
                m = await self._client.send_message(
                    self.bot_username, "/start nimbus init"
                )
            except Exception:
